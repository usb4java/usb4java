 ...
 * 
 * @deprecated Use the new libusb 1.0 API ({@link LibUSB} class) instead or
 *             consider using the JSR80 API which is independent of the
...
public final class USB
{
     ...
     */
    @Deprecated
    public static final int USB_REQ_CLEAR_FEATURE =
     ...
     */
    @Deprecated
    public static final int USB_REQ_GET_DESCRIPTOR =
     ...
     */
    @Deprecated
    public static final int USB_REQ_SET_DESCRIPTOR =
     ...
     */
    @Deprecated
    public static final int USB_REQ_GET_CONFIGURATION =
     ...
     */
    @Deprecated
    public static final int USB_REQ_SET_CONFIGURATION =
     ...
     */
    @Deprecated
    public static final int USB_REQ_GET_INTERFACE =
     ...
     */
    @Deprecated
    public static final int USB_REQ_SET_INTERFACE =
    ...
    public static final int USB_ENDPOINT_TYPE_CONTROL =
        LibUSB.TRANSFER_TYPE_CONTROL;// 0;
    ...
    public static final int USB_ENDPOINT_TYPE_ISOCHRONOUS =
        LibUSB.TRANSFER_TYPE_ISOCHRONOUS;// 1;
    ...
    public static final int USB_ENDPOINT_TYPE_INTERRUPT =
        LibUSB.TRANSFER_TYPE_INTERRUPT;// 3;
    ...
    /** Broken pipe. */
    private static final int EPIPE = 0;

    /** Interrupted system call. */
    private static final int EINTR = 32;
    ...
    /** The errno code of the last action. */
    private static int errno;

    /** The USB busses. */
    private static USB_Bus busses;
    ...
    /**
     * Converts a libusb 1.0 result code to an errno code used by libusb 0.1.
     * 
     * @param result
     *            The libusb 1.0 result code to convert.
     * @return The errno code.
     */
    private static int libUsbToErrno(final int result)
    {
        switch (result)
        {
            case LibUSB.SUCCESS:
    ...
    /**
     * Converts the specified libusb 1.0 result code to an errno code, remembers
     * it for the strerror method and returns the negative errno code which can
     * be used as a libusb 0.1 result code.
     * 
     * @param result
     *            The libusb 1.0 result code.
     * @return The libusb 0.1 result code.
     */
    static int err(final int result)
    {
        errno = libUsbToErrno(result);
        return -errno;
    }
    ...
    public static void usb_init()
    {
        if (context == null)
        {
            ...
            if (result < 0)
            {
                throw new RuntimeException("LibUSB.init() failed with code "
                    + result);
            }
        }
    }
    ...
    public static int usb_find_busses()
    {
        ...
        try
        {
            ...
            for (Device device: list)
            {
                ...
                // When first bus then use it as root
                if (root == null)
                {
                    root = new USB_Bus(busNum);
                    continue;
                }

                // When new bus then add it to the list
                if (!root.exists(busNum))
                {
                    root.add(new USB_Bus(busNum));
                }
            }

            // Calculate the difference between the previously found busses
            // and the now found busses
            int diff = busses != null ? busses.diff(root) :
                (root != null ? root.count() : 0);

            // Remember new bus list
            busses = root;

            // Return the number of new found busses
            return diff;
        }
        finally
        {
    ...
    public static int usb_find_devices()
    {
        ...
        try
        {
            // When no devices were found then we don't have to do anything
            if (result == 0) return 0;
            ...
            while (bus != null)
            {
                ...
                for (Device device: list)
                {
                    ...
                    // Ignore device when not connected to current bus
                    if (busNum != bus.location()) continue;

                    short devnum = (short) LibUSB.getDeviceAddress(device);

                    // Check if first device
                    if (bus.devices == null)
                    {
                        bus.devices = new USB_Device(device, bus, devnum);
                        result = bus.devices.init();
                        if (result < 0) return err(result);
                        changes++;
                        seen.add(devnum);
                        continue;
                    ...
                    // Add new device if not alredy present
                    if (!bus.devices.exists(devnum))
                    {
                        USB_Device dev = new USB_Device(device, bus, devnum);
                        bus.devices.add(dev);
                        result = dev.init();
                        if (result < 0) return err(result);
                        changes++;
     ...
     * 
     * @deprecated Use
     *             {@link LibUSB#open(de.ailis.usb4java.libusb.Device, de.ailis.usb4java.libusb.DeviceHandle)}
     */
    @Deprecated
    public static USB_Dev_Handle usb_open(final USB_Device device)
    {
        ...
        return new USB_Dev_Handle(handle);
    }
     ...
     * 
     * @deprecated Use
     *             {@link LibUSB#close(de.ailis.usb4java.libusb.DeviceHandle)}
     */
    @Deprecated
    public static int usb_close(final USB_Dev_Handle handle)
    {
        LibUSB.close(handle.handle);
        return 0;
    }
     ...
     * 
     * @deprecated Use
     *             {@link LibUSB#setConfiguration(de.ailis.usb4java.libusb.DeviceHandle, int)}
     */
    @Deprecated
    public static int usb_set_configuration(final USB_Dev_Handle handle,
        final int configuration)
    {
        return err(LibUSB.setConfiguration(handle.handle, configuration));
    }
     ...
     * 
     * @deprecated Use
     *             {@link LibUSB#setInterfaceAltSetting(de.ailis.usb4java.libusb.DeviceHandle, int, int)}
     */
    @Deprecated
    public static int usb_set_altinterface(final USB_Dev_Handle handle,
        final int alternate)
    {
        if (handle.lastClaimedInterface < 0)
            return -(errno = EINVAL);
        return err(LibUSB.setInterfaceAltSetting(handle.handle,
            handle.lastClaimedInterface, alternate));
    }
     ...
     * 
     * @deprecated Use
     *             {@link LibUSB#clearHalt(de.ailis.usb4java.libusb.DeviceHandle, int)}
     */
    @Deprecated
    public static int usb_clear_halt(final USB_Dev_Handle handle,
        final int ep)
    {
        return err(LibUSB.clearHalt(handle.handle, ep));
    }
     ...
     * 
     * @deprecated Use
     *             {@link LibUSB#resetDevice(de.ailis.usb4java.libusb.DeviceHandle)}
     */
    @Deprecated
    public static int usb_reset(final USB_Dev_Handle handle)
    {
        return err(LibUSB.resetDevice(handle.handle));
    }
     ...
     * 
     * @deprecated Use
     *             {@link LibUSB#claimInterface(de.ailis.usb4java.libusb.DeviceHandle, int)}
     */
    @Deprecated
    public static int usb_claim_interface(final USB_Dev_Handle handle,
        final int iface)
    {
        int result = LibUSB.claimInterface(handle.handle, iface);
        if (result == 0)
        {
            handle.lastClaimedInterface = iface;
        }
        return err(result);
    }
     ...
     * 
     * @deprecated Use
     *             {@link LibUSB#releaseInterface(de.ailis.usb4java.libusb.DeviceHandle, int)}
     */
    @Deprecated
    public static int usb_release_interface(final USB_Dev_Handle handle,
        final int iface)
    {
        int result = LibUSB.releaseInterface(handle.handle, iface);
        if (result == 0)
        {
            handle.lastClaimedInterface = -1;
        }
        return err(result);
    }
     ...
     * @param bytes
     *            The bytes to transfer. This must be a direct buffer obtained
     *            for example by ByteBuffer.allocateDirect().
     * @param timeout
     *            The timeout in milliseconds.
     * @return The number of bytes written/read or < 0 on error.
     * 
     * @deprecated Use
     *             {@link LibUSB#controlTransfer(de.ailis.usb4java.libusb.DeviceHandle, int, int, int, int, ByteBuffer, int)}
     */
    @Deprecated
    public static int usb_control_msg(final USB_Dev_Handle handle,
        final int requesttype, final int request, final int value,
        final int index, final ByteBuffer bytes, final int timeout)
    {
        int result = LibUSB.controlTransfer(handle.handle, requesttype,
            request, value, index, bytes, timeout);
     ...
     */
    @Deprecated
    public static int usb_get_string(final USB_Dev_Handle handle,
        final int index, final int langid, final ByteBuffer buffer)
    {
        int result = LibUSB.getStringDescriptor(handle.handle, index, langid,
            buffer);
        if (result >= 0) return result;
        return err(result);
    }
     ...
     * 
     * @deprecated There is no direct replacement for this. You may want to use
     *             {@link LibUSB#getStringDescriptor(de.ailis.usb4java.libusb.DeviceHandle, int, int, ByteBuffer)}
     *             .
     */
    @Deprecated
     ...
     */
    @Deprecated
    public static int usb_get_string_simple(final USB_Dev_Handle handle,
        final int index, final ByteBuffer buffer)
    {
        StringBuffer sbuffer = new StringBuffer();
        int result =
            LibUSB.getStringDescriptorAscii(handle.handle, index, sbuffer,
                buffer.capacity());
        if (result >= 0)
        {
            buffer.put(sbuffer.toString().getBytes());
            return result;
        }
        return err(result);
    }
     ...
     * 
     * @deprecated Use
     *             {@link LibUSB#getDescriptor(de.ailis.usb4java.libusb.DeviceHandle, int, int, ByteBuffer)}
     */
    @Deprecated
    public static int usb_get_descriptor(final USB_Dev_Handle handle,
        final int type, final int index, final ByteBuffer buffer)
    {
        int result = LibUSB.getDescriptor(handle.handle, type, index, buffer);
        if (result >= 0) return result;
        return err(result);
    }
     ...
     * @return Number of bytes read or < 0 on error.
     * 
     * @deprecated libusb 1.0 has no replacement for this method.
     */
    @Deprecated
    public static int usb_get_descriptor_by_endpoint(
        final USB_Dev_Handle handle, final int ep, final int type,
        final int index, final ByteBuffer buffer)
    {
        int result = LibUSB.controlTransfer(handle.handle,
            LibUSB.ENDPOINT_IN | (ep & 0xff), LibUSB.REQUEST_GET_DESCRIPTOR,
            (type << 8) | index, 0, buffer, 1000);
        if (result >= 0) return result;
        return err(result);
    }
     ...
     *            The endpoint.
     * @param bytes
     *            The bytes to write. This must be a direct buffer obtained for
     *            example by ByteBuffer.allocateDirect().
     * @param timeout
     *            The timeout in milliseconds.
     * @return Bytes written on success or < 0 on error.
     * 
     * @deprecated Use
     *             {@link LibUSB#bulkTransfer(de.ailis.usb4java.libusb.DeviceHandle, int, ByteBuffer, java.nio.IntBuffer, int)}
     */
    @Deprecated
    public static int usb_bulk_write(final USB_Dev_Handle handle,
        final int ep, final ByteBuffer bytes, final int timeout)
    {
        final IntBuffer transferred = IntBuffer.allocate(1);
        final int result =
            LibUSB.bulkTransfer(handle.handle, ep, bytes, transferred, timeout);
        if (result == 0) return transferred.get();
        return err(result);
    }
     ...
     *            The endpoint.
     * @param bytes
     *            The read bytes. This must be a direct buffer obtained for
     *            example by ByteBuffer.allocateDirect().
     * @param timeout
     *            The timeout in milliseconds.
     ...
     */
    @Deprecated
    public static int usb_bulk_read(final USB_Dev_Handle handle,
        final int ep, final ByteBuffer bytes, final int timeout)
    {
        final IntBuffer transferred = IntBuffer.allocate(1);
        final int result =
     ...
     * 
     * @deprecated Use
     *             {@link LibUSB#interruptTransfer(de.ailis.usb4java.libusb.DeviceHandle, int, ByteBuffer, java.nio.IntBuffer, int)}
     */
    @Deprecated
    public static int usb_interrupt_write(final USB_Dev_Handle handle,
        final int ep, final ByteBuffer bytes, final int timeout)
    {
        final IntBuffer transferred = IntBuffer.allocate(1);
        final int result =
            LibUSB.interruptTransfer(handle.handle, ep, bytes, transferred,
                timeout);
        if (result == 0) return transferred.get();
        return err(result);
    }
     ...
     */
    @Deprecated
    public static int usb_interrupt_read(final USB_Dev_Handle handle,
        final int ep, final ByteBuffer bytes, final int timeout)
    {
        final IntBuffer transferred = IntBuffer.allocate(1);
        final int result =
     ...
     * 
     * @deprecated Use {@link LibUSB#errorName(int)} with the error result code
     *             you received from a LibUSB function.
     */
    @Deprecated
     ...
     * @return 0 on success or < 0 on error.
     * 
     * @deprecated There is no direct replacement in libusb 1.0 but you can use
     *             {@link LibUSB#kernelDriverActive(de.ailis.usb4java.libusb.DeviceHandle, int)}
     *             to check if a kernel driver is currently active.
     */
    @Deprecated
    public static int usb_get_driver_np(final USB_Dev_Handle handle,
        final int iface, final ByteBuffer buffer)
    {
        int result = LibUSB.kernelDriverActive(handle.handle, iface);
        if (result == 1)
        {
            buffer.put("dummy\0".getBytes());
            return 0;
        }
        else if (result == 0)
        {
     ...
     */
    @Deprecated
    public static int usb_detach_kernel_driver_np(
        final USB_Dev_Handle handle, final int iface)
    {
        return err(LibUSB.detachKernelDriver(handle.handle, iface));
    }
     ...
     * 
     * @deprecated There is no replacment for this method in libusb 1.0. It
     *             always returns true now.
     */
    @Deprecated
    public static boolean libusb_has_get_driver_np()
    {
        return true;
    }
     ...
     *             method returns a corresponding error code. Because of this
     *             this method now always returns true.
     */
    @Deprecated
    public static boolean libusb_has_detach_kernel_driver_np()
    {
        return true;
    }
}
